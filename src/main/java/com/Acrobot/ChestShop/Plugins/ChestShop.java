...
public class ChestShop implements Listener {
	...
	public static void onProtectionCheck(ProtectionCheckEvent event) {
		if (event.getResult() == Event.Result.DENY || event.isBuiltInProtectionIgnored()) return;
	...
	public static boolean canAccess(Player player, Block block) {
		if (Permission.has(player, Permission.ADMIN) || !canBeProtected(block)) return true;
		
		if (isSign(block)) {
			Sign sign = (Sign) block.getState();
			
			if (!ChestShopSign.isValid(sign)) return true;
			
			if (!isShopMember(player, sign)) return false;
		}
		
		if (isChest(block)) {
			Sign sign = uBlock.getConnectedSign((Chest) block.getState());
			
			if (sign != null && !isShopMember(player, sign)) return false;
		}

...
public class PlayerInteract implements Listener {
	...
	public static void onInteract(PlayerInteractEvent event) {
		...
		if (Properties.USE_BUILT_IN_PROTECTION && isChest(block)) {
			if (Properties.TURN_OFF_DEFAULT_PROTECTION_WHEN_PROTECTED_EXTERNALLY) return;
		...
		if (!isSign(block) || player.getItemInHand().getType() == Material.SIGN) return;
		...
		if (ChestShopSign.canAccess(player, sign)) {
			if (!Properties.ALLOW_SIGN_CHEST_OPEN || player.isSneaking() || player.isInsideVehicle() || player.getGameMode() == GameMode.CREATIVE) return;
	...
	private static boolean isAllowedForShift(boolean buyTransaction) {
		...
		if (allowed.equalsIgnoreCase("ALL")) return true;
	...
	private static int getStackAmount(ItemStack item, Inventory inventory, Player player, Action action) {
		...
		if (checkedInventory.containsAtLeast(item, item.getMaxStackSize())) return item.getMaxStackSize();
		else return InventoryUtil.getAmount(item, checkedInventory);
	}
	...
	private static void showChestGUI(Player player, Block signBlock) {
		...
		if (!canOpenOtherShops(player) && !Security.canAccess(player, signBlock)) return;

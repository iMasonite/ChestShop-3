...
public class SignBreak implements Listener {
	...
	public static void onExplosion(EntityExplodeEvent event) {
		if (event.blockList() == null || !Properties.USE_BUILT_IN_PROTECTION) return;
	...
	private static List<Sign> getAttachedSigns(Block block) {
		if (block == null) return Lists.newArrayList();
		
		if (isSign(block)) return Arrays.asList((Sign) block.getState());
		else {

...
public class ServerAccountCorrector implements Listener {
	...
	public static void onCurrencyAdd(CurrencyAddEvent event) {
		...
		if (!ChestShopSign.isAdminShop(target) || event.getTarget().equals(SERVER_ECONOMY_ACCOUNT)) return;
	...
	public static void onCurrencyCheck(CurrencyCheckEvent event) {
		...
		if (!ChestShopSign.isAdminShop(target) || event.getAccount().equals(SERVER_ECONOMY_ACCOUNT)) return;

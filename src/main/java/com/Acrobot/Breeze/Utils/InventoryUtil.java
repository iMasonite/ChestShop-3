...
public class InventoryUtil {
	...
	public static int getAmount(ItemStack item, Inventory inventory) {
		if (!inventory.contains(item.getType())) return 0;
		
		if (inventory.getType() == null) return Integer.MAX_VALUE;
	...
	public static boolean isEmpty(Inventory inventory) {
		for (ItemStack stack : inventory.getContents()) {
			if (!MaterialUtil.isEmpty(stack)) return false;
		}
	...
	public static boolean fits(ItemStack item, Inventory inventory) {
		...
		if (inventory.getMaxStackSize() == Integer.MAX_VALUE) return true;
	...
	/** Adds an item to the inventory with given maximum stack size (it currently uses a custom method
	 * of adding items, because Bukkit hasn't fixed it for a year now - not even kidding)
	 * 

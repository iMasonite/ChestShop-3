...
public abstract class Database {
	...
	private void prepareDatabase(String driver, String url, String username, String password, String isolation) {
		...
		if (classes.isEmpty()) // Exception: There is no use in continuing to load this database
	...
	private void installDatabase(boolean rebuild) {
		...
		if (!rebuild && databaseExists) return;
			...
			while ((currentLine = scriptReader.readLine()) != null) {
				...
				if (currentLine.startsWith("create table")) {
					...
					if (alterTableLine[3].startsWith("add constraint")) {
						...
						else // Exception: This line cannot be fixed but is known the be unsupported by SQLite
	...
	/** Method called near the end of prepareDatabase, before the dataSourceConfig is attached to the
	 * serverConfig.
	 * 
